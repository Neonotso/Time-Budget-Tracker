...
def find_folder(name):
    q=f"trashed=false and name='{name}' and mimeType='{FOLDER}'"
    r=svc.files().list(q=q,fields='files(id,name)',pageSize=10).execute().get('files',[])
    return r[0] if r else None

folder = find_folder('WorkingHours')
if folder:
    print(f"Found folder: {folder['name']} ({folder['id']})")
    items=svc.files().list(q=f"trashed=false and '{folder['id']}' in parents",fields='files(id,name,mimeType,createdTime)',pageSize=300).execute().get('files',[])
    for it in sorted(items,key=lambda x:x['name']):
        print(f"- {it['name']} | {it['id']}")
else:
    print("Folder 'WorkingHours' not found.")
